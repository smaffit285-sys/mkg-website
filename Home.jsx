    ...
    <main className="overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 pb-12 px-4 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          <div className="mb-8 flex justify-center">
            <div className="relative w-40 h-40 md:w-52 md:h-52">
              <div className="absolute inset-0 rounded-full animate-pulse-slow"
                style={{ boxShadow: '0 0 40px rgba(255,45,120,0.6), 0 0 80px rgba(0,245,212,0.3)' }} />
              <img
                src="https://static.wixstatic.com/media/37d64c_c26d92e0c01a495caeb57ac548ba7b8f~mv2.jpg"
                alt="Miami Knife Guy"
                className="w-full h-full rounded-full object-cover border-2 border-neon-pink relative z-10"
              />
            </div>
          </div>
          ...
          {/* Headline */}
          <h1 className="font-heading font-black text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-tight mb-4">
          ...
          <div className="ticker-content">
            {[...tickerItems, ...tickerItems].map((item, i) => (
              <span key={i} className="inline-flex items-center gap-6 px-6">
                <span className="font-heading text-sm font-bold neon-pink tracking-wider">{item}</span>
              ...
              <div className="space-y-4">
                {painPoints.map((point, i) => (
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <p className="font-body text-light/70">{point}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="neon-border-cyan rounded-sm overflow-hidden bg-dark p-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center">
                    ...
                    />
                    <span className="font-heading text-xs text-neon-pink tracking-wider">DULL</span>
                  </div>
                  <div className="text-center">
                    <img
                      ...
                      className="w-full h-48 object-cover rounded-sm mb-2"
                    />
                    <span className="font-heading text-xs text-neon-cyan tracking-wider">SHARP</span>
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-neon-cyan/20 text-center">
                  <p className="font-mono text-xs text-neon-cyan/60">Every knife. Master standard. Every time.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-10">
            {lies.map((lie, i) => (
              <div key={i} className="card-dark rounded-sm p-5 group hover:border-neon-pink/40 transition-all">
                ...
                <h4 className="font-heading text-sm font-bold text-neon-pink mb-2">{lie.myth}</h4>
                <div className="h-px bg-neon-cyan/20 mb-3" />
                <p className="font-body text-light/60 text-sm leading-relaxed">{lie.truth}</p>
              </div>
            ))}
          </div>
          <div className="text-center">
            <p className="font-body text-light/50 mb-4">Ready to feel the difference? Your first knife is on us.</p>
      ...
      <section className="py-16 px-4 bg-dark border-y border-neon-cyan/10">
        <div className="max-w-4xl mx-auto text-center">
          <p className="font-mono text-neon-cyan text-xs tracking-[0.2em] uppercase mb-6">What Miami Kitchens Are Saying</p>
          ...
          <p className="font-mono text-xs text-muted/50">— Kitchen Manager, North Miami Beach · First service</p>
        </div>
      </section>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {['5★ Google Rated', 'Kosher & Halal', 'Free First Knife', '17 Years Experience'].map(badge => (
              <div key={badge} className="px-4 py-2 border border-neon-cyan/20 rounded-sm">
                <p className="font-mono text-xs text-neon-cyan/60 tracking-wider">{badge}</p>
              </div>
            ))}
          </div>
        </div>
